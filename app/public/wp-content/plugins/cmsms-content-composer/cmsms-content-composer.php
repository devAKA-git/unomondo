...
class Cmsms_Content_Composer { 
	function __construct() { 
		define('CMSMS_CONTENT_COMPOSER_VERSION', '1.4.2');
		...
		require_once(CMSMS_CONTENT_COMPOSER_PATH . 'inc/widgets.php');
		...
		require_once(CMSMS_CONTENT_COMPOSER_PATH . 'inc/twitter/twitteroauth.php');
...
";
	...
	function add_gutenberg_button() {
		...
		if (post_type_supports($post->post_type, 'editor')) {
			echo '<a href="#" id="cmsms_gutenberg_button" class="button button-large">' . __('&#8592; Back to Block Editor', 'cmsms_content_composer') . '</a>';
		}
	}
	...
	function show_cmsms_composer_meta_box() {
		...
		$gutenberg_show = get_post_meta($post->ID, 'cmsms_gutenberg_show', true);
		...
		'<div class="cmsms_composer_container">' . 
			...
			'<input type="hidden" id="cmsms_gutenberg_show" name="cmsms_gutenberg_show" value="' . (($gutenberg_show === 'true') ? 'true' : 'false') . '" />' . 

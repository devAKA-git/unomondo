				...
				$out .= '<li class="' . $new_gallery_col . '">' . 
					'<figure>';
						...
						if ($gallery_links != 'none') {
							...
							$out .= '<a' . (($gallery_links == 'blank') ? ' target="_blank"' : '') . ' href="' . $image_src[0] . '"' . (($gallery_links == 'lightbox') ? ' rel="ilightbox[' . $unique_id . ']"' : '') . ($image_caption != '' ? ' data-caption="' . esc_attr($image_caption) . '"' : '') . '>';
						}
						
						
						$out .= wp_get_attachment_image(strstr($image, '|', true), $image_size_gallery);
						...
						if (get_post_field('post_excerpt', strstr($image, '|', true)) != '') {
							$out .= '<figcaption>' . get_post_field('post_excerpt', strstr($image, '|', true)) . '</figcaption>';
						}
			...
			if ($tweets != '') {
				...
				echo '<div class="cmsms_notice cmsms_notice_error cmsms-icon-cancel-6">' . "\n" . 
					'<div class="notice_content">' . "\n" . 
						'<p>' . __('Please add your Twitter API keys', 'cmsmasters') . ', ' . '<a target="_blank" href="http://cmsmasters.net/twitter-functionality/">' . __('read more how', 'cmsmasters') . '</a></p>' . "\n" . 
					'</div>' . "\n" . 
				'</div>' . "\n";
			}

...
class Cmsms_Gutenberg {
	...
	public function __construct() {
		...
		add_action('register_post_type_args', array($this, 'cmsms_gutenberg_post_type_filter'), 10, 2);
	}
	
	
	public function cmsms_gutenberg_post_type_filter($args, $post_type) {
		if (
			$args['public'] == true && 
			(
		...
		<script id="composer-gutenberg-button-switch-mode" type="text/html">
			<div id="composer-switch-mode">
				<button id="composer-switch-mode-button" type="button" class="button button-primary button-large admin-icon-composer">
					<span class="composer-switch-mode-on"><?php _e( '&#8592; Back to Block Editor', 'cmsms_content_composer' ); ?></span>
	...
	public function set_is_composer_page( $post_id, $is_composer = true ) {
		if ( $is_composer ) {
			...
			delete_post_meta( $post_id, 'cmsms_gutenberg_show' );
		}
	}

	public function is_built_with_composer( $post_id ) {
		return get_post_meta( $post_id, 'cmsms_gutenberg_show', true );
	}

	...
	var ComposerLightbox = function (element, parameters) { 
		...
		obj.methods = { 
			...
			setVars : function () { 
				...
				obj.methods.lbHTML = '<div class="cmsmsBoxOut">' + 
					'<div class="cmsmsBoxBack"></div>' + 
					'<div class="cmsmsBoxCont">' + 
						'<div class="cmsmsBoxContIn admin-icon-loader animate-spin">' + 
							'<div class="cmsmsBoxContInTop wrap"></div>' + 
							'<div class="cmsmsBoxContInMid"></div>' + 
							'<div class="cmsmsBoxContInBot"></div>' + 
						'</div>' + 
					'</div>' + 
				'</div>';
			}, 
			...
			buildObj : function () { 
				if (obj.methods.options.closeButtons) {
					obj.methods.contInTop.append('<a href="#" class="cmsmsBoxClose admin-icon-remove" title="' + obj.methods.options.closeButtonText + '"></a>');
		...
		privateMethods = { 
			...
			attachGeneratedEvents : function () { 
				...
				if ($.inArray('table', obj.methods.eventsArray) !== -1) {
					...
					$('#cmsmsBox_' + obj.methods.uniqID + ' .cmsms_table').on('click', '.cmsms_table_cell.cmsms_change_cell', function () { 
						...
						$(this).removeClass('cmsms_change_cell').html('<textarea></textarea>').find('textarea').focus().html(cellText);
					} );
			...
			multiAddControls : function (el) { 
				el.prepend('<span class="cmsms_multi_handle admin-icon-move"></span>');
				
				
				el.append('<a href="#" class="cmsms_multi_del admin-icon-remove"></a>');
				
				
				el.append('<a href="#" class="cmsms_multi_copy admin-icon-copy"></a>');
			}, 

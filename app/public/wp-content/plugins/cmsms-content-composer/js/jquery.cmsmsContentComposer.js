...
(function ($) { 
	var ContentComposer = function (element, parameters) { 
		var defaults = { 
				...
				toGutenbergButton : 	'#cmsms_gutenberg_button' 
			}, 
			obj = this, 
			privateMethods = {};
		...
		obj.methods = { 
			...
			setVars : function () { 
				...
				obj.methods.gutenbergShow = $('#cmsms_gutenberg_show');
			...
			appendRowControl : function (el) { 
				...
				el.wrapInner('<div class="innerRow"></div>').prepend('<div class="innerHead">' + 
					...
					'<a href="#" class="cmsmsDelBut admin-icon-remove" title="' + cmsms_composer.remove_section + '"></a>' + 
					'<a href="#" class="cmsmsCopyBut admin-icon-copy" title="' + cmsms_composer.clone_section + '"></a>' + 
					'<a href="#" class="cmsmsEditBut admin-icon-edit" title="' + cmsms_composer.edit_section + '"></a>' + 
			...
			appendColumnControl : function (el) { 
				...
				'</div>').find('.innerColumn').append('<a href="#" class="cmsmsAddBut admin-icon-add" title="' + cmsms_composer.add_shortcode + '"></a>');
			}, 
			appendShCdControl : function (el) { 
				el.wrapInner('<div class="innerShortcode"></div>').prepend('<div class="innerHead">' + 
					'<a href="#" class="cmsmsDelBut admin-icon-remove" title="' + cmsms_composer.remove_shortcode + '"></a>' + 
					'<a href="#" class="cmsmsCopyBut admin-icon-copy" title="' + cmsms_composer.clone_shortcode + '"></a>' + 
					'<a href="#" class="cmsmsEditBut admin-icon-edit" title="' + cmsms_composer.edit_shortcode + '"></a>' + 
				'</div>').append('<a href="#" class="cmsmsEditShortcodeBut admin-icon-edit"></a>');
			}, 
			startObj : function () { 
				obj.methods.resetBlocks();
							...
							if (event.originalEvent.type === 'mouseup') {
								if (ui.draggable.hasClass('cmsms_row')) {
									...
									ui.draggable.html('<div class="cmsms_row cmsms_dropped hideEl"' + rowAttrs + '>' + 
										'<div class="cmsms_column one_first" data-width="1/1"></div>' + 
									'</div>');
				...
				obj.methods.el.sortable( { 
					...
					start : function (event, ui) { 
						var placeClasses = ui.item[0].className.replace(/cmsms_\S+/g, '').replace(/ui-\S+/g, '');
						...
						obj.methods.button_row.draggable( { 
							...
							start : function (event, ui) {
								$(ui.helper).attr('class', 'cmsms_row').css( { 
									...
									height: 75, 
									padding : '25px 0 0' 
								} ).addClass('ui-front').wrapInner('<div class="cmsms_row"></div>').empty().append('<div class="innerRow"></div>').find('.innerRow').height(63).append('<span class="cmsmsShortcodeTitle admin-icon-row"></span>').parent().prepend('<div class="innerHead">' + 
									'<span>' + cmsmsRow.title + '</span>' + 
								'</div>');
							} 
						} );
